...
from models.waypoint_transformer_cond import WaypointTransformer, WaypointTransformerConfig
...
def run_one_epoch(
    ...
    while True:
        ...
        with stopwatch.time("fps"):
            ...
            if policy.cfg.use_color:
                points = points.gather(1, fps_indices.unsqueeze(2).repeat(1, 1, 6))
            else:
                points = points.gather(1, fps_indices.unsqueeze(2).repeat(1, 1, 3))
            user_clicked_labels = user_clicked_labels.gather(1, fps_indices)

            # Normalize user click labels
            target_user_clicked = user_clicked_labels / user_clicked_labels.sum(dim=1, keepdim=True)
            
            # Randomly decide whether to mask salient point channel (50% of the batch)
            mask = (torch.rand(target_user_clicked.shape[0], device=target_user_clicked.device) < 0.5).float().unsqueeze(1)  # shape: [B, 1]
            # Apply mask: only retain labels for which we want to compute click loss
            masked_target_user_clicked = target_user_clicked * mask
            
            if dtype == torch.bfloat16:
                masked_target_user_clicked = masked_target_user_clicked.bfloat16()

            # Forward pass
            model_inp = torch.cat([points, masked_target_user_clicked.unsqueeze(-1)], dim=-1)
        ...
        with stopwatch.time("model"):
            with torch.autocast(device_type="cuda", dtype=dtype):
                (
                    ...
                    pred_gripper_logit,
                    pred_mode_logit,
                ) = policy.forward(model_inp, proprio)

                ### click ###
                per_sample_loss = F.cross_entropy(pred_clicked_logits, target_user_clicked, reduction="none")  # [B]
                click_loss = (per_sample_loss * (1.0 - mask.squeeze(1))).sum() / ((1.0 - mask).sum() + 1e-6)
...
def eval_inference_err(
    ...
    for data in dataset.datas:
        ...
        click_probs = torch.from_numpy(data["user_clicks"]).float()

        with torch.no_grad():
            pred_pos = policy.inference(xyz, color, click_probs, proprio, num_pass=num_pass)[1]
