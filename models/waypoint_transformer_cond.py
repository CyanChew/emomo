...
class WaypointTransformerConfig:
    ...
    topk_eval: int = 3
    # model cfg
    use_euler: int = 0  # euler or quat
    use_color: int = 1  # condition on color channel or not
    npoints: int = 1024
    pred_off: int = 1
    pred_point: int = 1
    per_point_rot: int = 0
...
class WaypointTransformer(nn.Module):
    def __init__(self, cfg: WaypointTransformerConfig):
        ...
        input_dim = 7 if self.cfg.use_color else 4
        self.input_embed = nn.Linear(input_dim, cfg.embed_dim)
        self.pos_rot_gripper_embed = nn.Parameter(torch.zeros(1, 3, cfg.embed_dim))
        self.proprio_embed = nn.Linear(11, cfg.embed_dim) # 3 EE Pos, 4 EE Quat, 1 Gripper, 3 Base Pose
        ...
        self.rot_dim =  3 if self.cfg.use_euler else 4
        if cfg.per_point_rot:
            self.points_output = nn.Linear(cfg.embed_dim, 3 + self.rot_dim + 1)  # offset + rot + click
        else:
            self.points_output = nn.Linear(cfg.embed_dim, 3 + 1)  # offset + click
        self.pos_output = nn.Linear(cfg.embed_dim, 3)
        self.rot_output = nn.Linear(cfg.embed_dim, self.rot_dim)
        self.gripper_output = nn.Linear(cfg.embed_dim, 1)
        self.mode_output = nn.Linear(cfg.embed_dim, 4) # ArmWaypoint, Dense, Terminate, BaseWaypoint
    ...
    def forward(
        ...
        if self.cfg.per_point_rot:
            click_logits, points_off, rot = points_out.split([1, 3, self.rot_dim], dim=2)
        else:
    ...
    #@torch.no_grad
    def inference(
       self, points: torch.Tensor, colors: torch.Tensor, click_probs: torch.Tensor, proprio: torch.Tensor, num_pass: int
        ...
        with torch.no_grad():
            ...
            click_probs = click_probs.unsqueeze(0)
            click_probs = click_probs / click_probs.sum(dim=1, keepdim=True)

            click_probs = click_probs.unsqueeze(2).repeat(num_pass, 1, 1).cuda()
            ...
            click_probs = click_probs.gather(1, fps_indices.unsqueeze(2))

            # points/colors: [num_pass, num_point, 6]
            if self.cfg.use_color:
                points = torch.cat([points, colors, click_probs], dim=2)
            else:
                points = torch.cat([points, click_probs], dim=2)

            proprio = proprio.unsqueeze(0).repeat(num_pass, 1).cuda()
            _, points_off, pos, rot, gripper_logit, mode_logit = self.forward(
                points, proprio
            )

            click_probs = click_probs.squeeze(-1)
            _, sorted_click_indices = click_probs.sort(dim=1, descending=True)
            ...
            if self.cfg.pred_off:
                if self.cfg.use_color:
                    ...
                    selected_points = points.gather(1, click_indices.unsqueeze(2).repeat(1, 1, 3))
            ...
            mode_probs = nn.functional.softmax(mode_logit[0], dim=-1)
